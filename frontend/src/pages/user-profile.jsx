...
const UserProfile = () => {
    ...
    return (
            <>
                <AddPaymentMethodModal setPaymentMethods={setPaymentMethods} setShowModal={setShowModal} showModal={showModal} />
                        ...
                        <div className="row g-4 justify-content-center">
                            ...
                            <div className="col-md-4">
                                <div className="div h-100 border p-2 text-center">
                                    ...
                                    <span className="fs-3 text-color">{user.successBuysCount + user.successSellsCount}</span>
                        ...
                        <div className="row justify-content-between">
                            ...
                            <div className=" mt-4 ">
                                {paymentMethods.map((method, index) => (
                                    <div className="payment__method-item p-3 d-flex align-items-center justify-content-between" key={method.id}>
                                        <div className="d-flex align-items-center">
                                            <div className="rounded-circle me-2 d-flex align-items-center justify-content-center"
                                                 style={{ width: '30px', height: '30px', backgroundColor:method.bank.color}}>
                                                <span className={` ${method.bank.color !== "yellow" ? "text-white": ""}`}>{method.bank.name.charAt(0)}</span>
                                            </div>
                                            <div className="d-block">
                                                <h6 className="text-color fw-bold m-0">{method.bank.name}</h6>
                                                <span className="secondary-text-color small">{method.cardHolderName}</span>
                                                <div></div>
                                                <span className="secondary-text-color small">{"**** **** **** " + method.lastFour}</span>
                                            </div>
                                        </div>
                                        <div>
                                            <button
