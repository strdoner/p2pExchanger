...
export default class OrderResponseService {
    static async createResponse(orderId: number, paymentMethodId: number): Promise<AxiosResponse<OrdersResponse>> {
        return $api.post(`orders/accept/${orderId}`, {paymentMethodId: paymentMethodId})
    }
